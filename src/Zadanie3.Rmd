...
# Clean and standardize the FAF and FCVC columns by rounding the values to the nearest whole number
...
# Create categorical factors from FAF, FCVC and Age
cleaned_data$FAF <- ifelse(cleaned_data$FAF == 0, 'No activity',
                            ifelse(cleaned_data$FAF == 1, 'Low activity',
                                   ifelse(cleaned_data$FAF == 2, 'Moderate activity', 'High activity')))

cleaned_data$FCVC <- ifelse(cleaned_data$FCVC == 1, 'Low consumption',
...
Here is the visual representation of the Bayesian Network based on the relationships we discussed. This graph illustrates how different factors such as Gender, Form of transformation (Trans), family history of overweight (ObesityFam), frequent high caloric food consumption (FAVC), frequency of vegetable consumption (FCVC), physical activity frequency (FAF) and the fact that individual is monitoring his calories (SCC) are hypothesized to influence obesity levels (Nobesity).
...
prob_trans <- prop.table(table(cleaned_data$MTRANS))
prob_obesityfam <- prop.table(table(cleaned_data$family_history_with_overweight))
prob_FAVC <- prop.table(table(cleaned_data$FAVC))
prob_FCVC <- prop.table(table(cleaned_data$FCVC))
prob_FAF <- prop.table(table(cleaned_data$FAF))
prob_scc <- prop.table(table(cleaned_data$SCC))
prob_calc <- prop.table(table(cleaned_data$CALC))
...
kable(prob_FAVC, caption = "Probability of Consumtion frequency of high caloric food", col.names = c("", "Probability"), format = "html", table.attr = "style='width:50%;'")  %>%
  kable_styling(position = "left")

kable(prob_FCVC, caption = "Probability of Consumption frequency of vegetables", col.names = c("", "Probability"), format = "html", table.attr = "style='width:50%;'")  %>%
  kable_styling(position = "left")

kable(prob_FAF, caption = "Probability of frequency of physical activity", col.names = c("", "Probability"), format = "html", table.attr = "style='width:50%;'")  %>%
  kable_styling(position = "left")

kable(prob_scc, caption = "Probability of monitoring calorie intake", col.names = c("", "Probability"), format = "html", table.attr = "style='width:50%;'")  %>%
  kable_styling(position = "left")

kable(prob_nobesity, caption = "Probability of Obesity", col.names = c("", "Probability"), format = "html", table.attr = "style='width:50%;'")  %>%
  kable_styling(position = "left")

kable(prob_calc, caption = "Probability of consumtion of alcohol", col.names = c("", "Probability"), format = "html", table.attr = "style='width:50%;'")  %>%
  kable_styling(position = "left")
```
...
![](C:/work/oznal/OZNAL_zadania/Data/vypocet1.pdf){width=100% height=1200}
...
![](C:/work/oznal/OZNAL_zadania/Data/vypocet2.pdf){width=100% height=1200}
...
![](C:/work/oznal/OZNAL_zadania/Data/vypocet3.pdf){width=100% height=2400}
...
![](C:/work/oznal/OZNAL_zadania/Data/vypocet4.pdf){width=100% height=2400}
...
![](C:/work/oznal/OZNAL_zadania/Data/vypocet5.pdf){width=100% height=3600}
